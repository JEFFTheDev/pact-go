...
$latestRelease = Invoke-WebRequest https://github.com/JEFFTheDev/pact-ruby-standalone/releases/latest -Headers @{"Accept"="application/json"}
$json = $latestRelease.Content | ConvertFrom-Json
$tag = $json.tag_name
$latestVersion = $tag.Substring(1)
$url = "https://github.com/JEFFTheDev/pact-ruby-standalone/releases/download/$tag/pact-$latestVersion-win32.zip"
...
$packages = go list github.com/JEFFTheDev/pact-go/... |  where {$_ -inotmatch 'vendor'} | where {$_ -inotmatch 'examples'}
$curDir=$pwd
